            ...
            <table id="datatable" class="table table-striped table-bordered">
                ...
                <?php if ($employeeList) { ?>
                    <?php foreach ($employeeList as $row) {
                        ...
                        <tr>
                            <td>
                                <a href="?view=employeeDetail&username=<?= $row->username; ?>"><?= __getName($row->username); ?></a>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-warning"
                                        onclick="location.href='?view=timesheetList&employee=<?= $row->username; ?>'">
                                    View Timesheets
                                </button>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-danger" href="#delete-alert-modal<?= $row->Id; ?>"
                                        data-animation="fadein" data-plugin="custommodal" data-overlayspeed="50"
                                        data-overlaycolor="#36404a">Terminate
                                </button>
                            </td>
                            </td>
                        </tr>
    ...
    <div id="delete-alert-modal<?= $row->Id; ?>" class="modal-demo">
        <div class="custom-modal-text">
            ...
            <button class="btn btn-sm btn-danger"
                    onclick="location.href='process.php?action=terminateEmployee&username=<?= $row->username; ?>&jobId=<?= $row->jobId; ?>&status=1'">
                Terminate
            </button>
